#!/usr/bin/env python3
"""YES leftover 3 machines on famous 2021–2022 dests not leftover-3× / not gold."""
...
real_spec = importlib.util.spec_from_file_location("real", ROOT / "scripts/implement-2018-2022-lo3x-three-machines.py")
...
YES = {
    "2021": "ios15 whatsapp21 telegram21 dalle1 codex win365 pixel6 robinhood paramount".split(),
    "2022": "discord netflix github notionai slack heardle quordle lockdown win22h2".split(),
}
...
def main() -> None:
        ...
        for slug in dests:
            ...
            for kind in ("first", "second", "third"):
                ...
                if re.search(pat, html, re.S):
                    ...
                    print("replaced", year, kind, slug)
                elif re.search(r"</body>", html, re.I):
                    html = re.sub(r"</body>", block + "\n</body>", html, count=1, flags=re.I)
                    print("inserted", year, kind, slug)
                else:
                    html += "\n" + block
                    print("appended", year, kind, slug)
                suffix = {"first": f"yeslo-{slug}", "second": f"yeslo2-{slug}", "third": f"yeslo3-{slug}"}[kind]
                verb, ph, keep, trap = real.truth(slug)
                rows.append(
                    {
    ...
    out = ROOT / "e2e/2018-2022-yes-leftover.matrix.json"
    out.write_text(json.dumps(rows, indent=2) + "\n")
    print("YES leftover matrix", len(rows))
