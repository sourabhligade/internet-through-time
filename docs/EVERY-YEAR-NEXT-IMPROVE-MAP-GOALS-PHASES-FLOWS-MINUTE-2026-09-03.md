...
```mermaid
flowchart TD
  land[Land dest] --> empty{Empty / trap / 0 ticks?}
  empty -->|wrote| failT[FAIL C4]
  empty -->|nothing| verb[Period verb]
  verb -->|ittYY leftover| next{Next this year 200?}
  next -->|404 / label mismatch| failN[FAIL I3]
  next -->|ok| gold{Star key unchanged?}
  gold -->|leftover wrote gold| failG[FAIL C13]
  gold -->|ok| done[Exit · ittYY-* only]
```
