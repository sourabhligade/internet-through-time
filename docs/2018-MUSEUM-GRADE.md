# 2018 — Museum grade card

**Date:** 2026-08-11  
**Status:** **Museum-grade A−** · lean from-scratch (2017 pattern) · hub **1994–2020**  
**Prefix:** `itt18` (reserved)  
**One-thing:** GDPR CMP Manage path · 25 May (not TikTok, not Face ID, not IGTV)  
**Game:** Consent Dash (CMP-class · parity with product Manage)

| Dimension | Status |
|-----------|--------|
| Research freeze | **[x]** READ-FIRST + harvest + from-scratch bible |
| Scaffold | **[x]** lean ~48 HTML · no HEAD forest |
| Shell | **[x]** Win10 mass · Chrome 68 · EdgeHTML |
| About scale | **[x]** 1,630,322,579 (−8% hostnames) · ITU 51.2% / 3.9B |
| P0 REAL | **[x]** GDPR gate · TikTok FYP · CA/hearing · IGTV · Spectre · HomePod · Chrome 68 |
| P1 | **[x]** YT Premium · FN Switch · GitHub · Spotify · FOSTA · G+ · Tumblr · iOS 12 |
| P2 harvest | **[x]** DBX · Oculus Go · TLS 1.3 · XS · Portal · Pixel 3 · Flickr 1000 · Edge announce |
| Game | **[x]** Consent Dash |
| e2e | **[x]** `test:e2e:2018` |
| Hub unlock | **[x]** 1994–2020 · 2019 and 2020 ARE on disk |
| Pixels | **[x]** failed-final text/CSS silhouettes |
| HEAD forest | **Do not restore** |

**Execute:** [`2018-FROM-SCRATCH-GOALS-PHASES-MINUTE-STEPS.md`](2018-FROM-SCRATCH-GOALS-PHASES-MINUTE-STEPS.md)  
**Facts:** [`2018-DEEP-RESEARCH-WEB-HARVEST-2026-08-11.md`](2018-DEEP-RESEARCH-WEB-HARVEST-2026-08-11.md)  
**Entry:** [`2018-READ-FIRST.md`](2018-READ-FIRST.md)

Still parked until a later year: Disney+ · Marshmello · Reels · Meta · Chromium Edge **as default** · official TikTok/Apple art.
