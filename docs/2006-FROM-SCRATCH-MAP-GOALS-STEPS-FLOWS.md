...
```mermaid
flowchart TD
  HUB["Hub card 2006 — locked today"] --> SHELL["XP desktop + IE 6"]
  SHELL --> HOME["Starting Point · guided 6 · star chip"]
  HOME --> ABOUT["1 About · June 85.5M · Nov 100M"]
  HOME --> GOLD["2 ★ Twttr 140"]
  HOME --> FEED["3 News Feed leftover"]
  HOME --> YT["4 YouTube leftover"]
  HOME --> DIGG["5 Digg leftover"]
  HOME --> MAP["6 Year map"]
  GOLD -->|"empty / 280 / For You"| NOWRITE["never writes"]
  GOLD -->|"2–140 + Update"| KEY["itt06-tweets"]
  KEY --> FEED
  FEED --> YT
  YT --> DIGG
  HOME --> REST["Reddit · Docs · AWS · Reader · Time You"]
  HOME --> GAME["TrailSled itt06-game-sled"]
  HOME --> POP["3× Bebo · SlideShare · Newsvine"]
  HOME --> BACK["Year menu → hub"]
```
