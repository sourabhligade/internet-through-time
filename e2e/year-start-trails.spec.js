...
test.describe("Per-year guided start trails", () => {
  test("hub registers 18 year chips and trail map has every YYYY-start", async ({ page }) => {
    await page.goto("/");
  ...
  test("deep link ?trail=2009-start writes night state", async ({ page }) => {
    await page.goto("/");
    await page.evaluate(() => {
      localStorage.removeItem("itt-first-night");
      localStorage.removeItem("itt-passport");
    });
    await page.goto("/years/2009/?trail=2009-start&room=pages%2Fabout.html");
