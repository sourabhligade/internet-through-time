...
test.describe("2015 densify", () => {
  test("home leftover 3× strips are unique doors", async ({ page }) => {
    ...
    const first = page.locator(`[data-itt-pop3x="2015"]`).first().locator('a[href*="sites/"]');
    const more = page.locator(`[data-itt-pop-more="2015"]`).first().locator('a[href*="sites/"]');
    const third = page.locator(`[data-itt-pop-3x3="2015"]`).first().locator('a[href*="sites/"]');
    if (await first.count()) await expect(first).toHaveCount(18);
    if (await more.count()) await expect(more).toHaveCount(18);
    if (await third.count()) await expect(third).toHaveCount(18);
  });
