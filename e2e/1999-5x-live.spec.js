...
test.describe('1999 5× live F1–F5', () => {
  ...
  test('F5 Y2K literacy empty never writes', async ({ page }) => {
    ...
    const raw = (await getKey(page, 'itt99-y2k')) || '';
    expect(raw).toMatch(/real|multiStep/i);
    const leak = await page.evaluate((yy) => {
      const bad = [];
      for (let i = 0; i < localStorage.length; i++) {
    ...
    expect(leak).toEqual([]);
    await expect(page.locator('[data-5x-next] a[href*="aim"]').first()).toBeVisible();
  });
