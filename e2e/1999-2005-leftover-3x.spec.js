// @ts-check
/**
 * Leftover-3× ×3 for 1999–2005 — dest-true · not mock.
 * Every MD door: empty never writes · trap never writes · complete writes
 * leftover JSON {real, leftover, multiStep, year} · star empty · neighbor empty.
 */
const { test, expect } = require("@playwright/test");
const ROWS = require("./1999-2005-leftover-3x.matrix.json");

const WANT = {
  1999: { first: 9, more: 9, third: 9 },
  2000: { first: 9, more: 9, third: 9 },
  2001: { first: 6, more: 6, third: 6 },
...
for (const row of ROWS) {
  test.describe(`${row.year} leftover-3× ${row.kind} ${row.id}`, () => {
    ...
    test(`M3 complete writes dest-true leftover · never gold`, async ({ page }) => {
      ...
      await panel.locator("[data-pop-field]").fill(row.ph);
      await go.click();
      const raw = await expect.poll(() => page.evaluate((k) => localStorage.getItem(k), row.key)).toBeTruthy();
      void raw;
