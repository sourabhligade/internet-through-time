// @ts-check
/**
 * Two more REAL leftover games per open year (extra-f / extra-g).
 * Finish-without-Start never writes. ?test=1 Start+Finish writes REAL.
 */
const fs = require("fs");
const path = require("path");
const { test, expect } = require("@playwright/test");
const matrix = require("./year-extra-fg.matrix.json").filter((row) =>
...
for (const year of Object.keys(byYear).sort()) {
  test.describe(`2 more games ${year}`, () => {
...
test("guided ol stays 6 after two-more pack", async ({ page }) => {
  for (const year of ["1994", "2008", "2018", "2024", "2025"]) {
