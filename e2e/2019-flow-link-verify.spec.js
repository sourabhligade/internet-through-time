...
test.describe("2019 flow-map + home links exist", () => {
  test("flow-map hrefs 200", async ({ page }) => {
    ...
    for (const rel of rels.slice(0, 40)) {
      const fromPages = rel.startsWith("../")
        ? `/years/2019/${rel.replace(/^\.\.\//, "")}`
        : `/years/2019/pages/${rel}`;
  ...
  test("guided 6 hrefs exist", async ({ page }) => {
    ...
    for (const href of hrefs) {
      if (!href) continue;
      const url = new URL(href, "http://x/years/2019/pages/home.html").pathname;
