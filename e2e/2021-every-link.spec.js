// @ts-check
/**
 * 2021 every dest link — dest URLs live (CUT-OPEN lean).
 */
...
test.describe("2021 every dest is live", () => {
  test("dest freeze 98", () => {
    expect(destFolders().length).toBe(98);
  });
  ...
  for (const dest of destFolders()) {
    ...
    test(`${dest} ${file} HTTP 200 · year 2021`, async ({ page }) => {
      expect(file, dest + " has index or game").toBeTruthy();
      const res = await page.goto(`/years/2021/sites/${dest}/${file}`);
...
test("2021 year shell is live Chrome habit not bounce", async ({ page }) => {
  await page.goto("/years/2021/");
  const skip = page.locator("#skip-connect");
  if (await skip.isVisible().catch(() => false)) await skip.click();
  await expect(page.locator("body")).toHaveAttribute("data-itt-year", "2021");
  await expect(page.locator("#content")).toBeVisible();
  await expect(page.locator("body")).not.toContainText(/2021 is not on the year menu/i);
});
