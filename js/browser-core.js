/**
 * Browser core — Netscape chrome controller (shared by all years)
 *
 * Usage:
 *   ITT.Browser.create(ITT.configs["1995"]);
 *
 * Depends on: js/lib/util.js
...
(function (global) {
  ...
  function create(config) {
     ...
     */
    var PERF = {
      navJitterMax: 48,          // was 180
      navFixedMax: 36,           // was 140
      historyNavMs: 28,          // back/forward snappy
      instantNavMs: 12,
      imageBudgetMs: 280,        // hard cap progressive reveal
      imageMinStepMs: 12,
      imageMaxStepMs: 48,
      imageStartMs: 8,
      singleImageMs: 40,
      connectBusyChance: 0.12,   // was 0.25
      connectEarlyMs: 160,       // was 320
      connectLineMs: 220,        // was 480
      connectBusyMs: 500,        // was 900
      connectEndMs: 180,         // was 400
      prefsPerfVersion: 2
    };
    if (config.perf) {
    ...
    var prefs = loadPrefs();
    // Migrate sticky old defaults that made the UI feel sluggish
    if (!prefs.perfVersion || prefs.perfVersion < PERF.prefsPerfVersion) {
      var legacySlow = { "1994": [350, 700], "1995": [80, 150, 350, 700] };
      var leg = legacySlow[YEAR] || [];
      if (leg.indexOf(Number(prefs.modemDelay)) !== -1) {
        prefs.modemDelay = (config.defaultPrefs && config.defaultPrefs.modemDelay != null)
          ? config.defaultPrefs.modemDelay
          : (YEAR === "1994" ? 160 : 45);
      }
    ...
    function defaultPrefs() {
      ...
      return {
        ...
        modemDelay: d.modemDelay != null ? d.modemDelay : 200,
    ...
    function navigate(path, options) {
      ...
      if (options.instant) {
        ...
        totalDelay = md <= 0 ? 0 : Math.min(PERF.historyNavMs, Math.max(12, Math.floor(md * 0.2)));
      } else if (md <= 0) {
        totalDelay = 0;
      } else {
        // Measured model: delay ≈ 0.65*md + small jitter (no +140 fixed tax)
        var jitter = Math.floor(Math.random() * Math.min(PERF.navJitterMax, Math.floor(md * 0.35) + 12));
        var fixed = Math.min(PERF.navFixedMax, Math.floor(md * 0.12));
        totalDelay = Math.floor(md * 0.65) + fixed + jitter;
      }

      var phases = [
        { t: 0, msg: "Contacting host: " + host + "..." },
        { t: Math.floor(totalDelay * 0.25), msg: "Host contacted. Waiting for reply..." },
        { t: Math.floor(totalDelay * 0.55), msg: "Reading file..." },
        { t: Math.floor(totalDelay * 0.8), msg: "Transferring data from " + host + "..." }
      ];
      ...
      // Start content load at ~40% of theater delay so chrome feels live sooner
      var startAt = totalDelay <= 0 ? 0 : Math.floor(totalDelay * 0.4);
      loadTimer = window.setTimeout(function () {
        ...
        setStatus("Transferring data from " + host + "...");
        setIframeSrc(path);
        fetch(absCheck, { method: "GET", cache: "no-store" })
          .then(function (res) {
            if (gen !== loadGen) return;
    ...
    function loadSingleImage(img) {
      ...
      var delay = Math.max(PERF.singleImageMs, Math.min(120, Math.floor((Number(prefs.modemDelay) || 80) * 0.35)));
    ...
    function applyProgressiveImages(doc) {
      ...
      // Hard budget: all images visible within imageBudgetMs regardless of count
      var budget = PERF.imageBudgetMs;
      // Slow modem pref scales budget up modestly (still capped)
      if (md >= 200) budget = Math.min(420, Math.floor(budget * 1.35));
      if (md >= 400) budget = Math.min(560, Math.floor(budget * 1.7));
      var step = Math.floor(budget / Math.max(1, list.length));
      step = Math.max(PERF.imageMinStepMs, Math.min(PERF.imageMaxStepMs, step));
      var revealed = 0;
      var totalK = Math.max(4, list.length * 12);
      ...
      function revealNext() {
        ...
        // Reveal in small batches when many images so budget holds
        var batch = list.length > 8 ? 2 : 1;
        ...
        setStatus("Transferring inline images: " + got + "K of " + totalK + "K");
        if (revealed >= list.length) {
          finishDocumentLoad(list.length);
          return;
        }
      ...
      setStatus("Transferring inline images...");
    ...
    function playModemSound(durationMs) {
      ...
      try {
        ...
        /* Phase 3: handshake noise (remaining time) */
      ...
      } catch (eAudio) { /* Audio not available — continue silently */ }
    }
    ...
    function connectSequence(busyFirst) {
      ...
      if (busyFirst) {
        lines.push("BUSY", "Retrying...", "ATDT 1-800-555-WEB1", "DIALING...");
      }
      lines = lines.concat([
    ...
    function seedHistory() {
      ...
      window.setTimeout(maybeFirstRunCoach, 400);
    }
    ...
    function maybePhoneEvent() {
      ...
      if (Math.random() > 0.035) return; // ~3.5% per navigation
      var kinds = [
      ...
      try { sessionStorage.removeItem(CONNECTED_KEY); } catch (e1) { /* */ }
      showAlert("Modem", msg);
